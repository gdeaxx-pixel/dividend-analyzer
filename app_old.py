# ══════════════════════════════════════════════════════════════════════════════
#  ARCHIVADO — esta fue la app de producción hasta el 2026-08-09.
#
#  El corte al port («Viaje del dinero») se hizo por renombre: este archivo era
#  `app.py` y el port era `app_v2.py`. Se conserva íntegro para poder revertir el
#  corte con `git revert` y como referencia de paridad. NO se ejecuta en Cloud.
#
#  OJO al leer `ui/**`: las referencias de la forma `app.py:NNNN` que aparecen en
#  esos módulos (y en `fixtures/`) apuntan a ESTE archivo — al `app.py` anterior al
#  corte, no al actual. Los números de línea son válidos aquí.
#
#  No lo edites: cualquier cambio de UI va en `app.py` + `ui/`. `logic.py` es
#  compartido y sigue siendo la fuente de cálculo.
#  Ver Obsidian/APPs/Logs/2026-08-09.md § Paso 3.
# ══════════════════════════════════════════════════════════════════════════════
...
# Modo demo local (?demo=ib|schwab|schwab2): carga un caso de real_examples/
# directo al paso 3, sin wizard. En producción no hay real_examples/ → inerte.
    ...
    try:
        ...
        if results:
            ...
            def _render_hoja_excel():
                ...
                _W = {'inicio': 'minmax(58px,0.85fr)', 'accion': '0.7fr',
                    ...
                    def _viaje_dinero():
                        ...
                        if hasattr(st, 'pills'):
                            ...
                            if _cprev.button('← Anterior', key=_sk + '_p', disabled=_step <= 0):
                                _step = max(0, _step - 1)
                            if _cnext.button('Siguiente →', key=_sk + '_n', disabled=_step >= 8):
                                _step = min(8, _step + 1)
                            st.session_state[_sk] = _step

                        # Riel de progreso — 9 marcas, azules hasta el paso actual.
                        _rail_html = ''.join(
                            f'<span style="display:inline-block;width:22px;height:3px;'
                            f'margin-right:6px;background:{"#006497" if _ri <= _step else "#dfe4e9"};">'
                            f'</span>' for _ri in range(len(_step_labels)))
                        st.markdown(f'<div style="margin:2px 0 14px 2px;">{_rail_html}</div>',
                                    unsafe_allow_html=True)
                        ...
                        # "Div. neto percibido" comparte el paso 2 con "Imp. NRA"; "Valor hoy"
                        ...
                        if not _no_imp:
                            _migas_defs.append(
                                (2, 'Div. neto percibido', _money(_d['neto']), False, False,
                                 '= Reinv + Efectivo'))
                        _migas_defs.append(
                            (3, 'Reinv + Efectivo', '', False, True,
                             f"{_money(_d['drip'])} + {_money(_d['cash'])}"))
                        _migas_defs.append(
                            (4, 'Capital trabajando', _money(_d['total']), False, True,
                             '= Bolsillo + Reinvertido'))
                        _migas_defs.append(
                            (5, 'Impacto del mercado',
                             ('−' + _money(-_d['mkt']) if _d['mkt'] < 0
                              else '+' + _money(_d['mkt'])),
                             _d['mkt'] < 0, True, ''))
                        _migas_defs.append(
                            (5, 'Valor hoy', _money(_d['mv']), False, False,
                             '= + Efectivo → Capital actual total'))
                        _migas_defs.append(
                            (6, '+ En efectivo', '+' + _money(_d['cash']), False, False, ''))
                        _migas_defs.append(
                            (6, 'Capital actual total', _money(_d['cat']), False, True,
                             '= Valor hoy + Efectivo'))
                        _migas_defs.append(
                            (7, '− Tu bolsillo', '−' + _money(_d['pocket']), False, False, ''))
                        if _has_rend:
                            _migas_defs.append(
                                (7, 'Resultado real',
                            ...
                            _migas_defs.append((7, 'Resultado real', '—', False, True, ''))

                        # Conector: un tick silencioso a la izquierda de cada chip, no una
                        ...
                        if _mkt < 0 and not _no_drip:
                            _n5 = (f'Aunque tu capital trabajando llegó a {_money(_d["total"])} '
                                   ...
                                   f'(<b>−{_mkt_neg_str}</b> de impacto del mercado). Hoy tus '
                                   f'acciones valen <b>{_money(_d["mv"])}</b>.')
                        elif _mkt >= 0 and not _no_drip:
                            _n5 = (f'Tu capital trabajando de {_money(_d["total"])} además se '
                                   f'apreció (<b>{_mkt_pos_str}</b> de impacto del mercado). Hoy '
                                   f'tus acciones valen <b>{_money(_d["mv"])}</b>.')
                            ...
                            _n5 = (f'Tus {_money(_d["pocket"])} cayeron con el precio de '
                                   ...
                                   f'Hoy tus acciones valen <b>{_money(_d["mv"])}</b>.')
                        else:
                            _n5 = (f'Tus {_money(_d["pocket"])} además se apreciaron '
                                   f'(<b>{_mkt_pos_str}</b> de impacto del mercado). Hoy tus '
                                   f'acciones valen <b>{_money(_d["mv"])}</b>.')
                        if _no_drip:
                            _n6 = (f'Sumando el valor actual de tus acciones '
                                   f'({_money(_d["mv"])}) más tu dinero disponible en efectivo, '
                                   f'aparte ({_money(_d["cash"])}), tu <b>capital actual '
                                   f'total</b> es {_money(_cat)}.')
                        else:
                            _n6 = (f'Sumando el valor actual de tus acciones '
                                   f'({_money(_d["mv"])}) más tu dinero disponible en efectivo '
                                   f'({_money(_d["cash"])}), tu <b>capital actual total</b> es '
                                   f'{_money(_cat)}.')
                        _n7 = (f'Frente a los {_money(_d["pocket"])} que pusiste de tu '
                        ...
                        _formulas = {
                            5: 'Capital trabajando + Impacto del mercado = Valor hoy',
                            6: 'Valor hoy + En efectivo = Capital actual total',
                            7: 'Capital actual total − Tu bolsillo = Resultado real',
                        }
                        if _step in _formulas:
                            _cur_html += (f'<span style="display:block;font-family:'
                        ...
                        if _step == 7:
                            ...
                            if _d.get('nav'):
                                ...
                                _cn_anchor2 = 'Compruébalo en el paso 9: Salud del NAV'
                            _cn_items = [
...
""", unsafe_allow_html=True)
                ...
                st.markdown(
                    ...
                    'color:#021C36;margin:6px 0 10px 0;letter-spacing:0.04em;">HOJA EXCEL · EL '
                    'MÉTODO TRADICIONAL VS LA REALIDAD</p>', unsafe_allow_html=True)

                # ── Tabla 1 — método tradicional (rejilla limpia, columnas comunes alineadas) ──
                _o1 = ['inicio', 'accion', 'inversion', 'netos', 'totalinv', 'valor']
                ...
                def _honest_grid():
                    ...
                    for r in _rows:
                        ...
                        _cells += _sp(_money(r['market_value'])) + _sp(_ret, _trc)
                    ...
                    _tc += _sp(_money(_t['market_value'])) + _sp(_tret, _trct)
                ...
                st.markdown(
                    ...
                    'margin:3px 0 14px 2px;line-height:1.5;">La hoja de cálculo de toda la vida suma '
                    '<b>Inversión + Dividendos</b> para decir cuánto «tienes invertido». Arriba la '
                    'replicamos tal cual (con su sesgo) y la comparamos con lo que de verdad pasó con tu '
                    'dinero. <b>Retorno total real</b> = Valor de mercado + dividendos en efectivo − tu '
                    'inversión. Es la única cifra que dice si ganaste o perdiste. Los «Div. netos» '
                    'incluyen lo <b>reinvertido (DRIP)</b>, que ya está dentro del «Valor de mercado»; '
                    'por eso el retorno suma solo el <b>efectivo</b>, para no contar el mismo dinero dos '
                    'veces. <b>Salud del NAV</b> juzga por la tendencia del precio del fondo (erosión), '
                    'no por el yield titular.</p>', unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
            ...
            if mode_a_tickers:
                _da_section('Portafolio dividendos')
...
<p style="font-family:Inter,sans-serif;font-size:10px;color:#445566;margin:4px 0 16px 0;">Base de Coste (ROC): el broker reduce el costo base por distribuciones clasificadas como Return of Capital. En Interactive Brokers: Portafolio → Posiciones → columna "Base de coste". En Charles Schwab: Cuentas → Posiciones → columna "Cost Basis".</p>
            ...
            # ── El yield anunciado vs lo que de verdad ganas ──────────
            if mode_a_tickers:
                _da_section('El yield anunciado vs lo que de verdad ganas',
                            'Un fondo puede anunciar \'80% de yield\' y aun así hacerte perder dinero. El '
                            'truco: el yield anualiza un solo pago de un flujo que salta cada semana, y no '
                            'descuenta la caída del precio.')
                _render_yield_audit(results, classify_map)
