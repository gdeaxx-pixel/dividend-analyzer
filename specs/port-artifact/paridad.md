...
| 21 | Total Return Graph — YieldMax vs Crecimiento (`app.py:5922`) | `ui/vistas.py:279` (ruta Comparación · Simulación) → `ui/componentes/render_comparacion` + `comparacion.html` (extraído por `tools/extract_comparacion.py`) | Fase 4: render medido con `ALTO_COMPARACION=920`, `initComparacion()` corre tal cual, sin adapter (no depende del CSV del usuario). **Re-medido en la auditoría del 2026-08-10:** `ALTO_COMPARACION` sube a 1100 — los 920 salían de medir solo a ancho de escritorio, y a 375px el panel real son 1025px (los chips envuelven en más filas). El auto-alto ya lo corregía en vivo; el respaldo estaba corto. |
| 21b | Total Return Graph con datos reales (`app_old.py:6013`, sección «07» — la UI que el port no había traído: `ui/vistas.py` la reimplementó con widgets nativos + Altair y quedó sin diseño del artifact) | `ui/vistas.py:187` `render_trg_real` (ruta Comparación · Real) → `ui/componentes/render_comparacion_real` + `comparacion_real.html` (derivado del panel de Simulación por `tools/extract_comparacion_real.py`), con datos de `ui/adapters.py:232` `trg_real_data` sobre `logic.build_drip_comparison_series` | Traspaso 2026-08-10, verificado en el navegador con `?demo=schwab2` (no por lectura de diff): panel `cmp-panel-real` con los mismos chips que Simulación; base por defecto **MSTY** (mayor YM del portafolio). Con base MSTY · DRIP bruto: **MSTY +23% · SCHB +56% · XLK +84% · SMH +173%** (SMH le gana a MSTY, igual que en el cálculo diario). Invariante fiscal por modo sobre MSTY: **plano −38% ≤ roc +2% ≤ bruto +23%**; SCHB/XLK/SMH dan idéntico en `roc` y `plano` (54/83/172) — correcto: sin avisos 19(a) el ROC-aware cae a tasa plana, y la nota del componente ya lo declara. Cambiar el fondo base a NVDY (→ NVDY +351%, SMH +303%) **no dispara rerun de Streamlit**: verificado marcando el nodo `<iframe>` y comprobando que sobrevive al cambio — la renormalización es JS (decisión 3). `ALTO_COMPARACION_REAL=1150` sobre 974px medidos a 1280 y 1086px a 375px. A 375px el desborde es **0** medido en el contenedor y en el documento. **0 errores de consola** (antes de esta sesión el componente de Comparación lanzaba `ReferenceError: POCKET is not defined` ×2 y `TypeError … reading 'hidden'` — ver H2/H3 del traspaso). Rama sin fixture forzada aparte: con `fetch_market_data` devolviendo `None` para CHPY, `trg_real_data` entrega 7 tickers y el componente filtra su chip (`_presente`), en vez de dibujarlo y reventar al pulsarlo. |
